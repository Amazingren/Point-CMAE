...
class GeoDecoder(nn.Module):
    ...
    def propagate(self, xyz1, xyz2, points1=None, points2=None):
        """
        Input:
        ...
        if S == 1:
            ...
            dists = torch.cdist(xyz1, xyz2).clip(min=0.0)
            dists, idx = dists.sort(dim=-1)
            dists, idx = dists[:, :, :self.de_neighbors], idx[:, :, :self.de_neighbors]  # [B, N, 3]

            dist_recip = 1.0 / torch.sigmoid(dists)
    ...
    def forward(self, xyz_list, point_list):
        xyz_list.reverse()
        point_list.reverse()
        x = point_list[0]
        for i in range(self.num_stages):
            # Propagate point features to neighbors
            x = self.propagate(xyz_list[i+1], xyz_list[i], point_list[i + 1], x)

        return x


def ot_assign(x, y, epsilon=1e-3, thresh=1e-3, max_iter=30, dst='eu'):
    device = x.device
