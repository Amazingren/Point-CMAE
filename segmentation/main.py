...
def parse_args():
    ...
    parser.add_argument('--deterministic', action='store_true', help='whether to set deterministic')
    ...
    parser.add_argument('--ckpts', type=str, default='../model_zoo/pretrain.pth', help='ckpts')
...
def main(args):
    ...
    if args.seed is not None:
        log_string(f'Set random seed to {args.seed + int(args.gpu)}')
        misc.set_random_seed(args.seed + int(args.gpu), deterministic=args.deterministic)
    ...
    testDataLoader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=args.batch_size * 2, shuffle=False,
                                                 num_workers=8)
    ...
    if args.test:
        ...
        test_metrics = validate(logger, num_part, classifier, testDataLoader, num_classes)
    ...
    def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
        ...
        for name, param in model.named_parameters():
            ...
            if 'conv' in name and "encoder" not in name:
                print("10 * LR: ", name)
                ...
                decay.append(param)
        return [
            {'params': finetune_head, 'lr': args.learning_rate * 10},
...
def validate(logger, num_part, classifier, testDataLoader, num_classes):
    def log_string(str):
        logger.info(str)
        print(str)
