    ...
    def forward(self, pts, img, text, **kwargs):
        ...
        if self.self_patch:
            with torch.no_grad():
                ...
                # Projection & Predict for teacher

            # --- DINO like cls loss ---
            # tem = 0.1
            teacher_cls = F.softmax((cls_token_k - self.center)/ 0.1, dim=-1).detach()
            cls_loss = torch.sum(-teacher_cls * F.log_softmax(cls_token / 0.1, dim=-1), dim=-1)
            ...
            # Projection & Predict for student
        ...
        if self.self_patch:
            with torch.no_grad():
                ...
                k_neighbor_project = self.projector_k(k_neighbor_feats)
                k_neighbor_project = F.normalize(k_neighbor_project, dim=-1)

            q_patch_project = self.projector_q(q_patch_feats)
            q_patch_predict = self.predictor_q(q_patch_project)
            q_patch_predict = F.normalize(q_patch_predict, dim=-1)

            k_neighbor_project = F.softmax(k_neighbor_project / 0.1, dim=-1)

            # TODO: The contrastive loss here is too simple,
            # Ccan try the more standard one: https://github.com/SwinTransformer/Transformer-SSL/blob/main/models/moby.py
            loss_selfpatch = torch.sum(-k_neighbor_project.detach() * F.log_softmax(q_patch_predict / 0.1, dim=-1), dim=-1)
