    ...
    def forward(self, pts, img, text, **kwargs):
        ...
        if self.self_patch:
            with torch.no_grad():
                ...
                k_neighbor_predict = self.linear_proj(k_neighbor_feats)

            q_patch_predict = self.linear_proj(q_patch_feats)
            q_patch_predict = F.normalize(q_patch_predict, dim=-1)

            k_neighbor_predict = F.softmax(k_neighbor_predict / 0.1, dim=-1)
            loss_selfpatch = torch.sum(-k_neighbor_predict.detach() * F.log_softmax(q_patch_predict / 0.1, dim=-1), dim=-1)
