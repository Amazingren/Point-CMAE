...
from utils.lib_loss import align_loss_3d, contrastive_loss_3d
from utils.lib_utilis import KNN, fps, index_points
...
class ReCon(nn.Module):
    def __init__(self, config):
        ...
        self.n_clusters = 56
        self.disdecoder = PointDisDecoder(self.trans_dim, mlp=[2*self.n_clusters, self.n_clusters])
    ...
    def forward(self, pts, img, text, **kwargs):
        ...
        # gt_points = neighborhood[mask].reshape(B * M, -1, 3) # Recon Style
        gt_points = neighborhood[mask].reshape(B, -1, 3)  # B M*group_size 3 # MAE Style

        cpts = center[mask].reshape(B, -1, 3)
        logits = self.disdecoder(gt_points, cpts, None, x_rec)
        loss_align_3d = align_loss_3d(gt_points, logits, tau=0.2, sink_tau=0.05, is_unin=True)
        losses['loss_align_3d'] = loss_align_3d
...
class PointDisDecoder(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            dists, idx = dists[:, :, :self.k], idx[:, :, :self.k]  # [B, N, 3]

            dist_recip = 1.0 / (dists + 1e-8)
            norm = torch.sum(dist_recip, dim=-1, keepdim=True)
            weight = dist_recip / norm
            interpolated_points = torch.sum(index_points(points2, idx) * weight.view(B, N, self.k, 1), dim=-2)
