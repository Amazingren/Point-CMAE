    ...
    def forward(self, pts, img, text, **kwargs):
        ...
        if self.self_patch:  
            ...
            with torch.no_grad():
                ...
                k_neighbor_proj = self.projector_k(k_neighbor_feats)
                k_neighbor_proj = F.normalize(k_neighbor_proj, dim=-1)

            # --- sp loss V1 --- 
            k_patch_pred = F.softmax(k_neighbor_proj / 0.1, dim=-1)
            loss_selfpatch = torch.sum(-k_patch_pred.detach() * F.log_softmax(q_patch_pred / 0.1, dim=-1), dim=-1)
