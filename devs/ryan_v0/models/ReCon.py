...
from utils.lib_loss import contrastive_loss_3d
...
class ReCon(nn.Module):
    ...
    # @torch.no_grad() # for moco loss
    # def _dequeue_and_enqueue(self, keys):
    #     # gather keys before updating queue
    #     # keys = dist_collect(keys) # only 1 gpu
    #     batch_size = keys.shape[0]
    #     ptr = int(self.queue_ptr)  # Convert torch.Tensor to integer if necessary
    #     # Calculate the indices to update in the queue
    #     indices = (torch.arange(batch_size) + ptr) % self.K
    #     # Transpose keys to align with the queue's dimensions
    #     keys = keys.T
    #     # Update the queue
    #     self.queue[:, indices] = keys
    #     # Move the pointer
    ...
    def forward(self, pts, img, text, **kwargs):
        ...
        if self.self_patch:
            q_patch_proj = self.projector_q(x_full)
            q_patch_pred = self.predictor_q(q_patch_proj)
            q_patch_pred = F.normalize(q_patch_pred, dim=-1)

            with torch.no_grad():
                self._momentum_update_key_encoder()  # update the key encoder
                cls_token_k, img_token_k, text_token_k, k_patch_feats, mask_k = self.MAE_encoder(
                    pts,
                    neighborhood,
                    center,
                    noaug=True
