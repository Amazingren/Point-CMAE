...
class ReCon(nn.Module):
    ...
    def forward(self, pts, img, text, **kwargs):
        ...
        if self.self_patch:
            ...
            cls_token_k = F.softmax(cls_token_k / 0.1, dim=-1).detach()

            loss_cls_dino = torch.sum(-cls_token_k * F.log_softmax(cls_token / 0.1, dim=-1), dim=-1)
            losses['dinocls_loss'] = loss_cls_dino.mean()
