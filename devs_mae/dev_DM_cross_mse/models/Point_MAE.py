...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        #    1-torch.cosine_similarity(de_feats_out1_co, de_feats_out2_co, dim=-1)
        # ).mean()
        loss_mse = (de_feats_out1 - de_feats_out2) ** 2 # [128, 64, 384]
        loss_mse = loss_mse.mean(dim=-1) # [128, 64]
        loss_contras = (loss_mse * comask).sum() / comask.sum()
