...
class MaskTransformer(nn.Module):
    ...
    def forward(self, neighborhood, center, noaug = False):
        ...
        return x_vis[:, 0], x_vis[:, 1:], bool_masked_pos
...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        q_cls_token, x_vis, mask = self.MAE_encoder(neighborhood, center)
        q_cls = F.normalize(q_cls_token, dim=1)
        ...
        with torch.no_grad():
            ...
            k_cls_topken, x_vis, mask = self.MAE_encoder_(neighborhood, center, noaug=True)
            k_cls = F.normalize(k_cls_topken, dim=1)

        l_pos = torch.einsum('nc, nc->n', [q_cls, k_cls]).unsqueeze(-1) # n 1 
        l_neg = torch.einsum('nc, ck->nk', [q_cls, self.queue.clone().detach()]) # n k
        ce_logits = torch.cat([l_pos, l_neg], dim=1)
        ce_logits /= self.T
        labels = torch.zeros(l_pos.shape[0], dtype=torch.long).to(pts.device)
        loss2 = self.loss_ce(ce_logits, labels)

        self._dequeue_and_enqueue(k_cls)
