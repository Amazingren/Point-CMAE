...
from models.helper import get_embedding_indices
...
class Point_MAE(nn.Module):
    def __init__(self, config):
        ...
        # prediction head
        self.n_k = max(3, int(self.mask_ratio * self.num_group * 0.1))
        self.disdecoder = PointDisDecoder(self.trans_dim, mlp=[2*self.proj_dim, self.proj_dim], K=self.n_k)

        self.center = torch.randn(self.n_clusters, self.proj_dim, requires_grad=False)
        self.center_momentum = 0.999
    ...
    def forward(self, pts, epoch, **kwargs):
        ...
        pos_emd = get_embedding_indices(center, sigma_d=0.5, sigma_e=0.1, angle_k=3, reduction_a='max')
        pos_emd = self.decoder_pos_embed(pos_emd).max(dim=-2)[0]
        # pos_emd_vis = self.decoder_pos_embed(center[~mask]).reshape(B, -1, C)
        # pos_emd_mask = self.decoder_pos_embed(center[mask]).reshape(B, -1, C)
        pos_emd_vis = pos_emd[~mask].reshape(B, -1, C)
        pos_emd_mask = pos_emd[mask].reshape(B, -1, C)
        ...
        # NOTES: x_rec measn x_all if N=-1
        x_rec = self.MAE_decoder(x_full, pos_full, -1)
        x_rec = self.norm(x_rec)

        M  = pos_emd_mask.size(1)
        ...
        self.center = self.center.to(pts)
        loss1 = align_loss_3d(logits, k_labels, self.center, tau=0.2, sink_tau=0.1)

        q_patch_feats = q_x_rec
        q_patch_norm = F.normalize(q_patch_feats, dim=-1)

        cdist = torch.cdist(cts_shift, cts_shift)
        ...
        q_neighbor_feats = torch.einsum('bnk,bnd->bnd', neighbor_weight, q_patch_feats)

        loss2 = contrastive_loss_3d(q_neighbor_feats, k_x.detach(), tau=0.1, is_norm=True)
