...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts_aug1, pts_aug2, vis = False, **kwargs):
        ...
        x_rec, feats_aug1_online = self.MAE_decoder(x_full, pos_full, N)
        B, M, C = x_rec.shape
        ...
        with torch.no_grad():
            ...
            _, feats_aug1_momentum = self.MAE_decoder_k(x_full_aug1, pos_full, N)

        # ***loss2***: Contrastive de_feats Loss:
        feats_aug1_online = F.normalize(feats_aug1_online, dim=-1)
        feats_aug1_momentum = F.softmax(feats_aug1_momentum / 0.1, dim=-1)
        loss_de = torch.sum(-feats_aug1_momentum.detach() * F.log_softmax(feats_aug1_online / 0.1, dim=-1), dim=-1)
        loss_de = loss_de.mean()
        ...
        # ***loss3***: Contrastive loss local: Feats()
        ...
        # ***loss4***: Contrastive loss global: cls_token
        ...
        loss_contras = 0.5 * loss_de + 1.0 * loss_contras_global
