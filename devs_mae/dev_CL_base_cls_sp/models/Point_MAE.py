...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts_aug1, pts_aug2, vis = False, **kwargs):
        ...
        with torch.no_grad():
            ...
            k_patch_feats = feats_aug1_momentum
            k_patch_norm = F.normalize(k_patch_feats, dim=-1)

            cdist = torch.cdist(center_aug1, center_aug1)
        ...
        # ***loss2***: Contrastive SP Loss:
        feats_aug1_online = F.normalize(feats_aug1_online, dim=-1)
        k_neighbor_feats = F.softmax(k_neighbor_feats / 0.1, dim=-1)
        loss_sp = torch.sum(-k_neighbor_feats.detach() * F.log_softmax(feats_aug1_online / 0.1, dim=-1), dim=-1)
        ...
        loss_contras = 0.1 * loss_sp + 1.0 * loss_contras_global
