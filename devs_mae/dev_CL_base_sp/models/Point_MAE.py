...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts_aug1, pts_aug2, vis = False, **kwargs):
        ...
        k_patch_feats = x_full_aug2
        k_patch_norm = F.normalize(k_patch_feats, dim=-1)
        cdist = torch.cdist(center_aug2, center_aug2)
        ...
        neighbor_aug2 = torch.einsum('bnk,bnd->bnd', neighbor_weight, k_patch_feats)

        # ***loss2***: Contrastive loss local: Feats()
        # x_full_aug2_pred = self.pred_feats(x_full_proj_aug2.permute(0, 2, 1))
        x_full_aug2_pred = self.pred_feats(neighbor_aug2.permute(0, 2, 1))
