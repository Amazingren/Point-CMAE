...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        cos_sim1 = torch.bmm(de_feats1_proj, de_feats1_proj.transpose(1, 2))
        cos_sim2 = torch.bmm(de_feats2_proj, de_feats2_proj.transpose(1, 2))
        mask_diag = torch.eye(64, dtype=torch.bool).unsqueeze(0).expand(B, -1, -1).to(mask1)  # (128, 64, 64)
        cos_sim1 = cos_sim1.masked_fill(mask_diag, 0)
        cos_sim2 = cos_sim2.masked_fill(mask_diag, 0)

        loss_contras = torch.sum(torch.abs(cos_sim1 - cos_sim2))
