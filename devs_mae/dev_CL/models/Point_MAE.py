...
class MaskTransformer(nn.Module):
    ...
    def forward(self, neighborhood, center, noaug = False):
        ...
        mask_idx1, vis_idx1 = torch.where(bool_masked_pos), torch.where(~bool_masked_pos)
...
class Point_MAE(nn.Module):
    def __init__(self, config):
        ...
        self.MAE_encoder_ = MaskTransformer(config)
        for param_q, param_k in zip(self.MAE_encoder.parameters(), self.MAE_encoder_.parameters()):
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        proj_cls_x, x_vis, mask = self.MAE_encoder(neighborhood, center)
        proj_cls_x = F.normalize(proj_cls_x, dim=1)

        B, N_vis, C = x_vis.shape  # B VIS C
        pos_emd_vis = self.decoder_pos_embed1(center[~mask]).reshape(B, -1, C)
        pos_emd_mask = self.decoder_pos_embed1(center[mask]).reshape(B, -1, C)
        _, N, _ = pos_emd_mask.shape
        mask_token = self.mask_token1.expand(B, N, -1)
        full_x = torch.cat([x_vis, mask_token], dim=1)
        pos_full_x = torch.cat([pos_emd_vis, pos_emd_mask], dim=1)

        # [bs, M1(38), 384], [128, 64, 384]
        x_rec, de_feats = self.MAE_decoder(full_x, pos_full_x, N)
        B, M, C = x_rec.shape
        ...
        if vis: #visualization
            ...
            return loss_recon, proj_cls_x, proj_cls_t, 
