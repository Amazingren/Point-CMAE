...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, **kwargs):
        neighborhood, center, neighborhood_aug, center_aug = self.group_divider(pts)
        ...
        with torch.no_grad():
            ...
            cls_token_k, x_vis_k, mask_k = self.MAE_encoder_k(
                neighborhood_aug, 
                center_aug, 
                noaug=True
            )
            ...
            cdist = torch.cdist(center_aug, center_aug)
            radius = torch.topk(cdist, k=5, dim=-1, largest=False)[0][:, :, 1:].mean(dim=-1, keepdim=True)
        ...
        loss_byol = loss_byol(q_patch_pred, k_patch_proj.detach())

        # # --- SP Loss:
        # k_patch_proj = F.softmax(k_neighbor_feats / 0.1, dim=-1)
        # sp_loss = torch.sum(-k_patch_proj.detach() * F.log_softmax(q_patch_pred / 0.1, dim=-1), dim=-1)
