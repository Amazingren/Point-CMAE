...
class Point_MAE(nn.Module):
    ...
    @torch.no_grad()
    def update_center(self, center, feats, labels):
        """
        Update center used for teacher output.
        """
        c_labels = torch.arange(self.n_clusters).long().to(center)
        new_feats = torch.cat((feats, center), dim=0)
        new_labels = torch.cat((labels, c_labels), dim=0).long()
        new_cluster_center = scatter_mean(new_feats, new_labels, dim=0, dim_size=self.n_clusters).detach() 

        # ema update
        self.center = (center * self.center_momentum + new_cluster_center * (1 - self.center_momentum)).detach_()
    ...
    def forward(self, pts, epoch, **kwargs):
        ...
        loss1, update_label = align_loss_3d(logits, k_labels, self.center, tau=0.2, sink_tau=0.1)
        loss2 = contrastive_loss_3d(x_rec, k_x_mask.detach(), tau=0.1, is_norm=True)

        with torch.no_grad():
            k_labels, update_label = k_labels.reshape(-1, self.proj_dim).contiguous(), update_label.reshape(-1).contiguous()
            self.update_center(self.center, k_labels, update_label)
