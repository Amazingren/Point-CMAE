...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            loss_1, proj_cls_x1, proj_cls_x2, proj_cls_x1_t, proj_cls_x2_t = base_model(points)

            T = next(sharpen_scheduler)
            proj_cls_x1 = F.normalize(proj_cls_x1, dim=-1)
            k_neighbor_feats = F.softmax(proj_cls_x1_t / T, dim=-1)

            loss_2 = torch.sum(-k_neighbor_feats.detach() * F.log_softmax(proj_cls_x1 / T , dim=-1), dim=-1)
            loss_2 = loss_2.mean()
