...
class MaskTransformer(nn.Module):
    ...
    def forward(self, neighborhood, center, noaug = False):
        ...
        if noaug == True:
            return proj_cls_x1, proj_cls_x2
        else:
...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        proj_cls_x1 = F.normalize(proj_cls_x1, dim=1)
        proj_cls_x2 = F.normalize(proj_cls_x2, dim=1)
        ...
        with torch.no_grad():
            proj_cls_x1_t, proj_cls_x2_t = self.MAE_encoder_(neighborhood, center, noaug=True)
            proj_cls_x1_t = F.normalize(proj_cls_x1_t, dim=1)
            proj_cls_x2_t = F.normalize(proj_cls_x2_t, dim=1)
        ...
        if vis: #visualization
            ...
            return loss_recon, proj_cls_x1, proj_cls_x2, proj_cls_x1_t, proj_cls_x2_t
...
class MMDLoss(nn.Module):
    ...
    def gaussian_kernel(self, x, y):
        ...
        return torch.exp(-torch.mean((tiled_x - tiled_y) ** 2, dim=2) / (2 * self.bandwidth ** 2))
    ...
    def forward(self, x, y):
        ...
        return torch.mean(kernel_x + kernel_y - 2 * kernel_xy)
