...
from utils.losses import contrastive_loss_3d
...
class Point_MAE(nn.Module):
    def __init__(self, config):
        ...
        # ViTs Encoder: encoder_q & encoder_k
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        with torch.no_grad():
            ...
            k_patch_proj = self.projector_k(x_all_k)

        # --- SP Loss:
        q_patch_pred = F.normalize(q_patch_pred, dim=-1, p=2)
        k_patch_proj = F.normalize(k_patch_proj, dim=-1, p=2).detach()

        sp_loss = torch.einsum('bnd,bnd->bn', [q_patch_pred, k_patch_proj]) / (torch.norm(q_patch_pred, dim=-1) * torch.norm(k_patch_proj, dim=-1))

        sp_loss = 2 - 2 * sp_loss.mean()

        # sp_loss = contrastive_loss_3d(k_patch_proj, q_patch_pred, tau=0.1, is_norm=False)
